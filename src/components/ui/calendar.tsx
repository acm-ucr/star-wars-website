  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        row: "flex w-full",
        cell: "border border-sw-gold-100 bg-white text-black relative p-0 text-center text-4xl focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected].day-range-end)]:rounded-r-md",
        day: cn(
      ...
      components={{
        ...
        IconLeft: ({ ...props }) => (
          <ChevronLeft className="h-7 w-7 stroke-sw-gold-100" />
        ),
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        IconRight: ({ ...props }) => (
          <ChevronRight className="h-7 w-7 stroke-sw-gold-100" />
        ),
      }}
      formatters={{
